...
// commitSimu calcule un engagement cm = MiMC(coins, energy, rho, rand).
func commitSimu(coins, energy, rho, r *big.Int) []byte {
...
// B        []byte
// G        bls12377.G1Affine
// buildEncMimcSimu : simule un chiffrement c = MiMC( pk, coins, energy, rho, rand, cm ) hors-circuit.
func buildEncMimcSimu(pk []byte, coins, energy, rho, rand *big.Int, cm []byte) []byte {
	h := mimcNative.NewMiMC()
	h.Write(pk)
	h.Write(coins.Bytes())
	h.Write(energy.Bytes())
...
func main() {
	...
	old1.Cm = commitSimu(old1.Value.Coins, old1.Value.Energy, big.NewInt(1111), big.NewInt(2222))
	...
	old2.Cm = commitSimu(old2.Value.Coins, old2.Value.Energy, big.NewInt(3333), big.NewInt(4444))
